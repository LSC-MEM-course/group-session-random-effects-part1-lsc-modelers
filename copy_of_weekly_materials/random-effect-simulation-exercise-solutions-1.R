...
simdata_randint$subject <- as.factor(simdata_randint$subject)
simdata_randint$subject_sum <- simdata_randint$subject
...
# combine and plot
nopool_estimates <- 
    summary(fit_nopool)$coef %>% as.data.frame %>%
    mutate(effect = rownames(.)) %>%
    filter(grepl("subject", effect)) %>%
    mutate(subject = levels(as.factor(simdata_randint$subject))[1:nrow(.)]) %>%
